...
class numPad(simpledialog.Dialog):
    ...
    def createWidgets(self):
        ...
        for label in btn_list:
            # function and argument will be taken care by partial
            cmd = lambda x = label: self.click(x)
            # now the part of creating the button
            cur = Button(self.top, text=label, width=10, height=5, command=cmd)
            btn.append(cur)
            # position the button
            btn[-1].grid(row=r, column=c)
            # Button index for the increment
            n += 1
            # update the row positon or column position
            c += 1
            if c == 3:
                c = 0
                r += 1
